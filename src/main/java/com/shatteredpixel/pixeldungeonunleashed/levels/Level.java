...
public abstract class Level implements Bundlable {
	...
	public Heap drop( Item item, int cell ) {
		...
		//This messy if statement deals with items which should not drop in challenges primarily.
		if ((Dungeon.isChallenged( Challenges.NO_FOOD ) && (item instanceof Food || item instanceof YumyuckMoss.Seed)) ||
		...
		if ((map[cell] == Terrain.ALCHEMY) && (
				!(item instanceof Plant.Seed || item instanceof Yumyuck) ||
				item instanceof YumyuckMoss.Seed ||
				(item instanceof Yumyuck && (((Yumyuck) item).potionAttrib != null || heaps.get(cell) != null))||
	...
	public Plant place( Plant.Seed seed, int pos ) {
	...
	public boolean[] updateFieldOfView( Char c ) {
		...
		if (c.isAlive()) {
			...
			} else if (c == Dungeon.hero && ((Hero)c).heroClass == HeroClass.THACO) {
				for (Mob mob : mobs) {
					int p = mob.pos;
	...
	public String tileDesc( int tile ) {
		...
		case Terrain.ALTAR:
			return "An ancient stone Altar for Herbert.";
		default:
			if (tile >= Terrain.WATER_TILES) {
