...
import com.shatteredpixel.pixeldungeonunleashed.effects.BannerSprites.Type;
...
import com.shatteredpixel.pixeldungeonunleashed.windows.WndChallenges;
import com.shatteredpixel.pixeldungeonunleashed.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		String TXT_NEW_DIF;
		switch (GoblinsPixelDungeon.getDifficulty()) {
			case 9:
		...
		HeroClass[] classes = {
				HeroClass.COMPLAINS, HeroClass.CHIEF, HeroClass.FUMBLES, HeroClass.THACO
		};
		...
		curClass = null;
		updateClass( HeroClass.values()[GoblinsPixelDungeon.lastClass()] );
	...
	private void updateClass( HeroClass cl ) {
		...
		if (cl != HeroClass.THACO || huntressUnlocked) {
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( GoblinsPixelDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(GoblinsPixelDungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( GoblinsPixelDungeon.challenges() > 0 ?
