...
import com.shatteredpixel.pixeldungeonunleashed.effects.Fireball;
import com.shatteredpixel.pixeldungeonunleashed.ui.Archs;
import com.shatteredpixel.pixeldungeonunleashed.ui.ExitButton;
import com.shatteredpixel.pixeldungeonunleashed.ui.PrefsButton;
...
public class TitleScene extends PixelScene {
    ...
    private static final String TXT_LOADSAVE		= "Load";
	
	@Override
	public void create() {
		super.create();

		int gameversion = GoblinsPixelDungeon.version();
		...
		float height = title.height +
				(GoblinsPixelDungeon.landscape() ? DashboardItem.SIZE : DashboardItem.SIZE * 2);
		...
		DashboardItem btnBadges = new DashboardItem( TXT_BADGES, 3 ) {
			...
			protected void onClick() {
				GoblinsPixelDungeon.switchNoFade( BadgesScene.class );
			}
		};
		add( btnBadges );
		...
		DashboardItem btnAbout = new DashboardItem( TXT_ABOUT, 1 ) {
			@Override
			protected void onClick() {
				GoblinsPixelDungeon.switchNoFade( AboutScene.class );
			}
		};
		add( btnAbout );
		...
		DashboardItem btnPlay = new DashboardItem( TXT_PLAY, 0 ) {
			@Override
			protected void onClick() {
				GoblinsPixelDungeon.switchNoFade( StartScene.class );
			}
		};
		add( btnPlay );
		...
		DashboardItem btnHighscores = new DashboardItem( TXT_HIGHSCORES, 2 ) {
			@Override
			protected void onClick() {
				GoblinsPixelDungeon.switchNoFade( RankingsScene.class );
			}
		};
		add( btnHighscores );
		/*
		DashboardItem btnLoadSave = new DashboardItem( TXT_LOADSAVE, 4 ) {
			@Override
			protected void onClick() {
                // TODO: Work out how to correctly load that LoadSaveScene.class, without crashes.
				GoblinsPixelDungeon.switchNoFade( LoadSaveScene.class );
			}
		};
		add( btnLoadSave );
	*/
		if (GoblinsPixelDungeon.landscape()) {
         ...
         //   btnLoadSave.setPos( w / 2 - (btnBadges.width()/2) , btnAbout.top() + DashboardItem.SIZE );
		}
