...
public abstract class Char extends Actor {
	...
	public void move( int step ) {
		...
		if (Level.adjacent( step, pos ) && buff( Euphoria.class ) != null) {
			step = pos + Level.NEIGHBOURS8[Random.Int( 8 )];
            if (!(Level.passable[step] || Level.avoid[step]) || Actor.findChar( step ) != null) {
                return;
            }
            if (this == Dungeon.hero && Dungeon.hero.belongings.armor != null) {
                Dungeon.hero.belongings.armor.doUnequip(Dungeon.hero, true, false);
            }
			...
			if (this == Dungeon.hero) {
				...
				if (Random.Float() < (0.20f )) {
					...
					Dungeon.level.drop(item, passable.get(Random.Int(passable.size()))).sprite.drop(pos);
                    items.remove(item);
				}
			} else {
				if (Random.Float() < (0.20f )) {
					Dungeon.level.drop(Generator.random(), passable.get(Random.Int(passable.size()))).sprite.drop(pos);
				}
			}
		}
