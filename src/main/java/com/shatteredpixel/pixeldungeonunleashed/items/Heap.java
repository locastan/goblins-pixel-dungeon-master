...
public class Heap implements Bundlable {
	...
	public Item transmute() {
		...
		if (items.size() == 2 && items.get(0) instanceof Seed && items.get(1) instanceof Yumyuck) {

			Sample.INSTANCE.play( Assets.SND_PUFF );
			CellEmitter.center( pos ).burst( Speck.factory( Speck.EVOKE ), 3 );

			Yumyuck result = new Yumyuck();
		...
		if (bonus != -1 ? alchemy.tryCook(count) : count >= SEEDS_TO_POTION) {
			...
			while (potion instanceof PotionOfHealing && (Random.Int(Dungeon.limitedDrops.cookingHP.count + 1) > 3))
				potion = Generator.random( Generator.Category.POTION );
